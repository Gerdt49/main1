...
class WardrobeTest {
    ...
    void testAreaComparator(){
        ...
        Comparator<Wardrobe> comparator = (w1, w2) -> Double.compare(w1.area(), w2.area());// Создаем компаратор для сравнения объектов класса Wardrobe по площади
        Arrays.sort(wardrobes,comparator);// Сортируем массив "wardrobes" с использованием созданного компаратора
        printArray(wardrobes, "Test Area Comparator after sort");
        int index = Arrays.binarySearch(wardrobes,new Wardrobe(2,4,1));// Ищем индекс объекта Wardrobe с заданными размерами (2, 4, 1) в отсортированном массиве
        System.out.println("index = " + index );}
    @Test
    ...
    void testHeightVolumeComparator(){
        printArray(wardrobes, "Test HeightVolume Comparator before sort");
        ...
        Arrays.sort(wardrobes,comparator);
        printArray(wardrobes, "Test HeightVolume Comparator after sort");
        int index = Arrays.binarySearch(wardrobes,new Wardrobe(2,4,1));
        System.out.println("index = " + index );
    }
}